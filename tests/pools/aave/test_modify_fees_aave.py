...
def test_commit(alice, swap, fee, admin_fee, offpeg):
    ...
    assert swap.future_offpeg_fee_multiplier() == offpeg


def test_commit_only_owner(bob, swap):
    with brownie.reverts("dev: only owner"):
        swap.commit_new_fee(23, 42, 31337, {"from": bob})
...
def test_commit_offpeg_too_high(alice, swap, fee, offpeg):
    with brownie.reverts("dev: offpeg multiplier exceeds maximum"):
        swap.commit_new_fee(fee, 0, offpeg // fee + 1, {"from": alice})
...
def test_apply(chain, alice, swap, fee, admin_fee, offpeg):
    ...
    assert swap.offpeg_fee_multiplier() == offpeg


def test_apply_only_owner(chain, alice, bob, swap):
    swap.commit_new_fee(0, 0, 0, {"from": alice})
    chain.sleep(COMMIT_WAIT)
